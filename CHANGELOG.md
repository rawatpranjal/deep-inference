...
- **Docs sidebar: single-click pages (econirl style).** Set `titles_only: True` in the RTD theme options so the sidebar stops spilling each page's in-page H2/H3 headings into the nav as fake dropdowns. Single pages are now one click; only Models, Inference, and Theory expand, to their child pages. Also moved the Models toctree above its H2 sections so the 16 model pages render at nav depth 2 (they were truncated at depth 3 and missing from the sidebar).
- **Docs refinement v2: flatten + consolidate (MECE, single-click pages).** Per the new goal, removed the sidebar logo (`html_logo`), and made the menu flat: only Models, Inference, and Theory remain multipart (dropdowns); every other section is a single-click long page. Consolidated the split sections into substantial single pages: Guide (folded the flowchart, the Lambda/diagnostics tables, and the three worked applications into `docs/guide.md`), Simulation Studies (the 9 evals + verification into `docs/simulation_studies.md`), API Reference (8 module pages into `docs/api.md`). Rebuilt Theory into two deep tracks, `theory/flm.md` (linear/logit focus, multinomial demoted) and `theory/riesznet.md` (RieszNet theory), grounded in cited transcript line-ranges. Kept Inference lean (just enough to implement; the deep math now lives in Theory). MECE split enforced: Theory = the math/guarantees, Inference = how-to, Estimation = pipeline, Models = model + estimand. Archived all superseded pages to `archive/docs_legacy/` (nothing deleted), repointed every internal link, made Replications single-click (DiD scoping page kept as an orphan reachable by inline link). Build 0 warnings, shipped docs dash-free.
...
- Added `docs/dev/competitors.md`, an internal decider doc mapping the deep causal inference landscape (PyWhy/DoWhy/EconML, DoubleML, Uber CausalML, causallib) against `deep-inference`. Coverage matrix over 16 method families, per-package profiles verified from each package's own docs, and a gap analysis showing the white space is torch-native estimators with influence-function SEs across causal and structural-econ targets. Ends with a build-next shortlist cross-linked to `backlog.md` and grounded in the 86-paper literature corpus. Competitor cells verified from each package's own docs, deep-inference cells from the package code, and the load-bearing corpus-paper characterizations (DragonNet, DeepIV, DeepGMM, RieszNet) grounded in their on-disk full text rather than abstracts.
...
- **Variance unification + dead-knob cleanup (Batch 3).** Added one shared SE/CI estimator `engine.variance.compute_se_ci(psi, fold_indices, n, method, alpha)` (`method='pooled'` = FLM Bessel sample variance of ψ centered at the global mean; `method='within_fold'` = legacy per-fold-centered variant; z = `norm.ppf(1-α/2)` for both, no hardcoded 1.96) and routed BOTH entry points through it. **`structural_dml` default SE switched from within-fold to pooled** (FLM-prescribed; pooled ≥ within-fold so CIs only widen slightly). Threaded a public `variance='pooled'|'within_fold'` knob through `structural_dml`, `structural_dml_core`, `run_crossfit`, and `inference()`. Killed the dead matrix-inversion `ridge` knob from the public signatures of `inference`/`run_crossfit`/`compute_psi` (it flowed into `batch_inverse`, whose TIKHONOV default ignores it) and exposed `tikhonov_scale=0.01` as the real inversion knob. Lambda-ESTIMATION `ridge`/`ridge_alpha=1000` (a different, correct knob) left untouched; `core/algorithm.py`'s inversion `ridge` left as-is (out of scope, also inert under TIKHONOV). Verified byte-identical at defaults: inversion `max|old-new|=0.0`; engine pooled SE float32→float64 refinement reldiff `1.1e-8` (formula identical). New `tests/test_variance_unification.py` (7 tests); full target suite 30 green.
- **API coherence (Batch 4).** Kept the three entry points (`structural_dml`/`inference`/`did`) per decision; made them coherent rather than deprecating any. Added a "Which entry point?" decision tree + `did()` to the module docstring; added the 8 missing public names (`DiDModel`, `FEPanelDiDModel`, `CombinatorialModel`, `DoseResponse`, `Profit`, `TailProbability`, `ConditionalVariance`, `MultiTreatmentATE`) to `__all__`; gave `inference()` a function-local `DiDModel`/`FEPanelDiDModel` import (no more module-global fallback) + a full model/target list in its docstring; `structural_dml` now auto-bumps `patience`→50 for `multinomial_logit` only (the documented fatal case; default stays 10 elsewhere). 23 fast tests green.
- **Doc sync (Batch 5).** Fixed drift in `CLAUDE.md` Package Structure tree: removed phantom `models/custom.py` and `targets/custom.py` (CustomModel/model_from_loss live in `models/base.py`, CustomTarget in `targets/base.py`), added the DiD family (`_did_closed.py`, `models/did.py`, `models/panel_fe.py`, `utils/fe.py`), the 3 economic targets (`targets/elasticity.py`, `wtp.py`, `welfare.py`), corrected `average_parameter.py`→`average_param.py`, and completed the `autodiff/` and `utils/` blocks. `README.md`: added a "Which entry point?" decision tree, a Difference-in-Differences section (`did()` exact/neural/panel_fe), and quantile + combinatorial mentions. `docs/api/targets.md`: added Elasticity/WTP/ConsumerWelfare sections + table rows + target-string list. `docs/api/index.md`: added `did()` to Quick Reference, refreshed targets/models module summaries. Relocated planning docs to `docs/dev/` (`full_requirements_next_gen.md`, `paper_replication_details.md`, `smoke_e2e_user_run_tests.md`, `new_content.md`→`backlog.md`); `handoff.md` and `smoke_test.py` left in place.
- **Pristine close-out (pre-share audit), Batch 0+1.** Fixed the stale non-editable install (package now `pip install -e .` editable on py3.11; `import deep_inference` resolves to `src/` with `did` present). Correctness: `inference(model='multinomial_logit', target='beta')` was **silently returning E[α₂] instead of E[β₁]** (hardcoded `param_index=1`, dims dropped) — now threads `n_alternatives`/`n_attributes` into the model and resolves `beta` to `param_index=(J-1)+target_idx` (E2E: `mu_hat≈-0.84` vs `E[β₁]=-0.8`). `inference()` now raises `ValueError` on unexpected kwargs (was silently dropping them). Deleted dead `compute_psi_vmap`; added fail-loud `hessian_depends_on_y` guard to `ComputeLambda`/Regime-A `select_lambda_strategy`; trimmed unreachable target-map branches; fixed stale `did.py` docstring. 23 fast tests pass; `git diff` confirms variance/linalg/ridge_alpha untouched (validated logit path byte-identical — no regression). Verified by 3 independent subagents.
- Consolidated the DiD API into a **single `did()` entry point** (auto-selects `method='exact'|'neural'|'panel_fe'` from the arguments). Replaces the separate `did_2x2`/`did_2x2_nn`/`did_panel_fe` functions. Closed-form core moved to `_did_closed.py` (renamed from `did.py`) to free the `did` name. Registered models (`model='did'`, `model='did_fe'`) unchanged. Docs section in `docs/api/inference.md` consolidated.
- Added two-way fixed-effects panel DiD: `did_panel_fe(Y, D, X, unit, time)` / `model='did_fe'`. Within-transform (residualize Y and D by unit+time FE via new `utils/residualize_fixed_effects`) then neural `Ỹ=D̃·τ(X)+ε`; estimates E[τ(X)] (Regime B, intercept-free analytic Λ=E[D̃²|X] — added an `intercept` flag to `AnalyticLambda`, routed via `BaseModel.analytic_intercept`). Works for continuous and binary (LPM, HC-robust IF SE). New `FEPanelDiDModel` (`models/panel_fe.py`). Firewall `evals/eval_15_panel_fe.py` (M=50): recovery corr 0.88, autodiff/lambda exact, **coverage 98% (continuous) / 94% (binary)**, all metrics PASS. Consolidated study `simulations/sim_04_did.py` (recovery + coverage across the DiD family). Cluster-robust SE intentionally deferred (SE assumes iid errors). Documented in `docs/api/inference.md`.
...
- Populated Sim 2 CNN+Logit results: 85% coverage (FAIL), SE ratio 0.83, |bias| 0.015, z-mean -0.63. Updated discussion to address CNN undercoverage (regularization bias at n=10,000). Sim 3 SBERT+Logit still running in background (M=20, n=10,000).
