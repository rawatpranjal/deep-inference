...
\begin{itemize}
    \item \texttt{user\_embeddings.npy}: $(5{,}000 \times 64)$ consumer embeddings
    \item \texttt{item\_embeddings.npy}: $(90 \times 64)$ item embeddings
\end{itemize}
...
\begin{itemize}
    ...
    \item $T_i$: Packed alternative attributes. For each choice occasion, we sample $J = 20$ alternatives (1 chosen + 19 random). Each alternative has $K = 6$ attributes: log-price and 5 PCA dimensions of the item embedding (64D $\to$ 5D via PCA, capturing $\sim$70\% of variance). The packed array is $T_i \in \mathbb{R}^{120}$ (reshaped to $20 \times 6$ inside the loss).
...
Before applying the method to real data, we validate it on a simulation DGP that mirrors the application specification ($J = 20$, $K = 6$, $d_x = 10$). We run $M = 50$ Monte Carlo replications at $n = 5{,}000$.
...
\midrule
Coverage (95\% CI) & [to be filled] & 90--99\% \\
SE ratio            & [to be filled] & 0.7--1.5 \\
$|$Bias$|$          & [to be filled] & $< 0.05$ \\
$z$-mean            & [to be filled] & $(-0.3, 0.3)$ \\
$z$-std             & [to be filled] & 0.7--1.5 \\
\bottomrule
\end{tabular}
\begin{tablenotes}\small
\item Note: Results from \texttt{application/06\_simulation\_study.py} with $M=50$, $n=5{,}000$, $J=20$, $K=6$.
\end{tablenotes}
\end{table}
...
\midrule
$E[\beta_{\text{price}}]$ & [to fill] & [to fill] & [to fill] & [to fill] & [to fill] \\
$E[\beta_{\text{pca}_1}]$ & [to fill] & [to fill] & [to fill] & [to fill] & [to fill] \\
$E[\beta_{\text{pca}_2}]$ & [to fill] & [to fill] & [to fill] & [to fill] & [to fill] \\
$E[\beta_{\text{pca}_3}]$ & [to fill] & [to fill] & [to fill] & [to fill] & [to fill] \\
$E[\beta_{\text{pca}_4}]$ & [to fill] & [to fill] & [to fill] & [to fill] & [to fill] \\
$E[\beta_{\text{pca}_5}]$ & [to fill] & [to fill] & [to fill] & [to fill] & [to fill] \\
\bottomrule
\end{tabular}
\begin{tablenotes}\small
\item Note: IF SE accounts for neural network estimation uncertainty. Ratio = IF SE / Naive SE. Results from \texttt{application/03\_inference.py} with $n\_folds=50$, $epochs=300$, $patience=50$.
\end{tablenotes}
\end{table}

The IF standard errors are systematically larger than naive SEs, reflecting the additional uncertainty from neural network estimation. This is exactly the pattern predicted by theory: naive SEs ignore the estimation step and produce overconfident intervals.
