...
\begin{enumerate}
    \item \textbf{Representation learning.} We train two-tower contrastive embeddings \citep{oord2018representation} to learn dense representations of consumers and products from purchase histories and visual features. Consumer embeddings capture latent taste dimensions; item embeddings capture visual and categorical product attributes.

    \item \textbf{Choice model estimation.} Consumer embeddings become the covariates $X$ that drive heterogeneity. Each consumer has their own price sensitivity $\theta_{\text{price}}(X_i)$ and taste parameters $\theta_{\text{style},k}(X_i)$. The neural network maps the 64-dimensional consumer embedding to these structural parameters.

    \item \textbf{Influence function inference.} The \texttt{deep-inference} package handles cross-fitting, Lambda estimation, influence function assembly, and confidence interval computation. With a five-line custom loss function, the entire inference pipeline runs automatically.

    \item \textbf{Counterfactual analysis.} We compute average price elasticities with valid confidence intervals and simulate the revenue impact of a 10\% price increase---with uncertainty bands that properly account for the neural network estimation step.
\end{enumerate}
...
Throughout, we emphasize the \emph{practitioner's} perspective: what choices must be made, what can go wrong, and what to look for in the output. We use the first person and aim for clarity over formality.
