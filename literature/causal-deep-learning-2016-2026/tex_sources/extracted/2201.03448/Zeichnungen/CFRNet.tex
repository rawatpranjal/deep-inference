...
\begin{document}
	...
	\begin{tikzpicture}[x=1.5cm, y=1.5cm, >=stealth]
		...
		\node [latent layer,fill=green!10] (out) at (3,0) {$\Phi$};
...
%		\draw [-] (E) -- (7.55,1.2)--(7.55,-0.5);
		...
		%Treatment Row
		\draw [->] (out.south) -- (3,-1.7)--(7.75,-1.7);
		\draw [->] (0,-1.9) --(3.5,-1.9) --(7.75,-1.9);
		\node [single node] (T) at (0,-1.8) {};
		\node [single node] (IPM) at (8,-1.8) {};
		\draw [->] (3.5,-1.9) -- (p);
		\node (x) at (-0.5,-1.8) {T};
		\node (lossIPM) at (9.8,-1.8) {$\text{IPM}_\epsilon^p(\Phi^{T=1},\Phi^{T=0})$};
		
		

		%Loss functions
		\node (loss1) at (10,0.5) {$L^{T=1}(h_1(\Phi(\textbf{x})),Y^{T=1})$};
		\node (loss0) at (10,-0.5) {$L^{T=0}(h_0(\Phi(\textbf{x})),Y^{T=0}$)};			
