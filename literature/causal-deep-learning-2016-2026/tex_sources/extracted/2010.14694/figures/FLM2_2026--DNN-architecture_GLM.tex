...
%				\node[input neuron] (I-2) at (0,-2) {$x_2$};
				...
				\node[input neuron] (I-3) at (0,-3.5) {$x_d$};
				...
				\node[yshift=10pt, parameter neuron, right of=Hb-2] (P-1) { \ $\ahat(\bx)$ \ };
				\node[yshift=-10pt, parameter neuron, right of=Hb-4] (P-2) { \ $\bbhat(\bx)$ \ };
			 
			 
			
			    % Draw the model layer nodes
			    \node[output neuron, right of=Hb-2, xshift=\layersep] (T) {$\bt$};
%			    \node[output neuron, right of=Hb-4, xshift=\layersep] (O) { \ $G\big(\ahat(\bx)\!+\!\bbhat(\bx) ' \bt\big)$ \ };
                \node[output neuron, pin={[pin edge={->}]right:$\widehat{y} = G(\cdot) $}, right of=Hb-4, xshift=\layersep] (O) { \  \ };
