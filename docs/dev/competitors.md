...
- **Automatic debiasing / Riesz.** Riesz regression (2104.14737), RieszNet and ForestRiesz (2110.03031), and moment-constrained debiasing (2409.19777). This is the most natural extension of our influence-function machinery and has no production home anywhere. The `auto-dml` and `riesz` tags cover these.
- **Representation CATE with valid CIs.** TARNet and CFRNet (1606.03976), DragonNet (1906.02120). CausalML ships DragonNet but as a point estimator. The gap is the same architecture with honest standard errors. The `representation` and `dragonnet` tags cover more than thirteen papers.
