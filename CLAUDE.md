...
**Coverage lesson (folds + repeats).** Linear ATE coverage improves with `n_folds>=50` (the docstring's own recommendation; `n_folds=20` under-covers) plus repeated cross-fitting `n_repeats>=5` (median DML, which is the SE-ratio lever). At n=2000 this reaches ~94% point coverage / SE-ratio ~0.915. CAVEAT: those are M=200 point estimates (coverage SE ~1.7pp), so do not over-read small coverage gaps.

**Root cause of the residual SE undercount (2026-06-27, RESOLVED).** It is NOT finite-sample and NOT nuisance overfitting. **Most of the problem is in estimating and inverting Λ(x).** The two-way path (linear's default, since the squared-error Hessian is θ-independent) collapses the covariate-varying curvature Λ(x)=2[[1,e(x)],[e(x),e(x)]] to a single global-mean matrix, i.e. the propensity e(x)=E[T|X] becomes the constant ē. Confirmed three ways: (a) flat in n (SE-ratio 0.886 at n=8000 vs 0.899 at n=2000), (b) closed form (flat weight 1/V̄=4.0 vs correct E[1/(e(1-e))]=7.44; predicted flat SE 0.0461 = observed 0.0463), (c) the oracle Λ(x) injected into the real pipeline restores it (variance ratio Var(ψ̂)/Var(ψ*)=1.000, SE-ratio into target). The load-bearing object is Λ⁻¹, not Λ: regressing the per-obs Hessian entries independently and inverting is unstable because Λ(x) is near-singular at low overlap (det=4e(1-e)→0) — ridge fits Λ well (R²=0.94) but its inverse explodes (Λ⁻¹-R²=−19807). FLM(correct Λ) ≡ RieszNet (the correction weight (T-e)/(e(1-e)) is exactly the Riesz representer). Fix direction under study: estimate the scalar propensity e(x) directly (well-conditioned) and form Λ(x) analytically, rather than regressing Hessian entries and inverting. Diagnostic harness: `exploration/lambda_decomp.py` (full 11-object fit panel); injection hook `lambda_eval_fn` + exposed `result.lambda_hat` in `core/algorithm.py`.
